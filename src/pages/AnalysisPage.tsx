...
function hasAnyOverlap(msgs: Pos[], gap: number): boolean {
  for (let i = 0; i < msgs.length; i++) {
    for (let j = i + 1; j < msgs.length; j++) {
      if (
        rectsOverlap(
          toRect(msgs[i].left, msgs[i].top),
          toRect(msgs[j].left, msgs[j].top),
          gap,
        )
      )
        return true;
...
function runSimulation(params: SimParams) {
  ...
  // ── 2. 겹침 발생률 ───────────────────────────────────────
  // 메시지 수 n = 1~msgCount에서 simCount회 반복 → 겹침이 한 번이라도 발생한 비율(%)
  const overlapRandom: number[] = [];
  const overlapPoisson: number[] = [];
  for (let n = 1; n <= msgCount; n++) {
    let rCnt = 0,
      pCnt = 0;
  ...
  // ── 4. 밀집도별 평균 시도 횟수 ───────────────────────────
  // 기존 메시지 수(fill)를 0→(msgCount-1)로 늘려가며, 새 메시지 1개를 배치하는 데
  // 필요한 평균 시도 횟수를 100회 평균으로 측정 → 알고리즘 성능 지표
  const fillLevels: number[] = [];
  const avgAttempts: number[] = [];
  for (let fill = 0; fill <= msgCount - 1; fill++) {
    let total = 0;
    const TRIALS = 100;
...
export default function AnalysisPage() {
  ...
  useEffect(() => {
    ...
    const distLabels = Array.from(
      { length: BIN_N },
      (_, i) => `${Math.round((i * MAX_DIST) / BIN_N)}`,
    );
      ...
      chartsRef.current[0] = new Chart(scatterRef.current, {
        ...
        data: {
          datasets: [
            {
              label: "랜덤",
              data: d.scatterRandom.map((m) => ({
                x: Math.round((m.left / 100) * SCREEN_W),
                y: Math.round(m.top),
              })),
    ...
    if (overlapRef.current) {
      chartsRef.current[1] = new Chart(overlapRef.current, {
        type: "line",
        data: {
          labels: Array.from({ length: params.msgCount }, (_, i) => i + 1),
          datasets: [
            {
              label: "랜덤",
              data: d.overlapRandom,
        ...
        options: {
          ...
          scales: {
            ...
            y: {
              min: 0,
              max: 100,
              title: { display: true, text: "겹침 발생률 (%)" },
            },
          },
        },
      });
    ...
    if (attemptsRef.current) {
      chartsRef.current[3] = new Chart(attemptsRef.current, {
        ...
        options: {
          ...
          scales: {
            x: {
              title: { display: true, text: "기존 메시지 수 (화면 밀집도)" },
            },
  ...
  const sliders: {
    ...
    { key: "msgCount", label: "화면 메시지 수", min: 5, max: 70, step: 1 },
    ...
    <div className={styles.page}>
      <div className={styles.header}>
        ...
        <p className={styles.subtitle}>
          랜덤 배치 vs Poisson Disk Sampling 기반 배치 비교
        </p>
      </div>
        ...
        {[
          {
            ref: scatterRef,
            title: "공간 분포 시각화",
            desc: "같은 수의 메시지를 배치했을 때의 위치 분포 차이",
          },
          {
            ref: overlapRef,
            title: "겹침 발생률 비교",
            desc: "메시지 수가 늘어날수록 겹침이 얼마나 발생하는가",
          },
          {
            ref: minDistRef,
            title: "최소 거리 분포",
            desc: "시뮬레이션별 메시지 간 최소 거리 분포 (클수록 여유있음)",
          },
          {
            ref: attemptsRef,
            title: "배치 시도 횟수 vs 화면 밀집도",
            desc: "화면에 메시지가 많을수록 빈 자리 찾는 데 시도가 몇 번 필요한가",
          },
        ].map((card) => (
